...
class AINetworkInterface:
    ...
    def send_direction_change(self, direction):
        ...
        if self.train_name in self.room.game.trains and self.room.game.is_train_alive(self.train_name):
...
class AIClient:
    ...
    def __init__(self, room, name):
        ...
        self.network = AINetworkInterface(room, name)  # Use AI name for network interface
        
        # Create agent
        self.agent = Agent(name, self.network, "server.ai_agent", False)  # Use AI name for agent
    ...
    def run(self):
        ...
        while self.running and self.room.running:
            try:
                ...
                if not self.game.trains[self.agent_name].alive and self.agent.waiting_for_respawn:
                    ...
                    if elapsed >= self.agent.respawn_cooldown:
                        logger.debug(f"AI client {self.agent_name} respawn cooldown over, checking game state")
                        if self.in_waiting_room:
                            logger.debug(f"AI client {self.agent_name} in waiting room, trying to start game")
                            # Start game if in waiting room
                            if not self.room.game_thread or not self.room.game_thread.is_alive():
