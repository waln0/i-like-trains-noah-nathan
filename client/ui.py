...
class UI:
    ...
    def get_player_ids(self):
        ...
        # Variables for input
        input_box_name = pygame.Rect(self.client.screen_width / 2 - 100, self.client.screen_height / 2 - 40, 200, 32)
        input_box_sciper = pygame.Rect(self.client.screen_width / 2 - 100, self.client.screen_height / 2 +40, 200, 32)
        color_inactive = GRAY
        color_active = BLUE
        ...
        while not (train_name_done and sciper_done):
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        if text_name:
                            ...
                            if self.client.network.check_name_availability(text_name):
                                ...
                                train_name_done = False
                                error_message = f"The train name '{text_name}' is invalid"
                        else:
                            train_name_done = False
                        ...
                        if text_sciper:
                            ...
                            if self.client.network.check_sciper_availability(text_sciper):
                                sciper_done = True
                            else:
                                sciper_done = False
            ...
            title_rect = title.get_rect(center=(self.client.screen_width / 2, self.client.screen_height / 2 - 100))
            self.client.screen.blit(title, title_rect)
            
            # Label for name
            name_label = small_font.render("Name:", True, BLACK)
            name_label_rect = name_label.get_rect(topleft=(input_box_name.x, input_box_name.y - 25))
            ...
            self.client.screen.blit(text_surface, (input_box_name.x + 5, input_box_name.y + 5))
            
            # Label for sciper
            sciper_label = small_font.render("Sciper:", True, BLACK)
            sciper_label_rect = sciper_label.get_rect(topleft=(input_box_sciper.x, input_box_sciper.y - 25))
            ...
            self.client.screen.blit(text_surface, (input_box_sciper.x + 5, input_box_sciper.y + 5))
            
            # Error message
            if error_message:
                error_surface = small_font.render(error_message, True, RED)
                error_rect = error_surface.get_rect(center=(self.client.screen_width / 2, self.client.screen_height / 2 + 120))
                self.client.screen.blit(error_surface, error_rect)
                
            # Instructions
            instructions = small_font.render("Press Enter to validate", True, BLACK)
            instructions_rect = instructions.get_rect(center=(self.client.screen_width / 2, self.client.screen_height / 2 + 100))
